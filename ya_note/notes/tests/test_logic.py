...
class NoteTestCase(TestCase):
    ...
    def test_note_creation_and_automatic_slug_generation(self):
        test_cases = [
            {'login': True, 'title': 'Test Note 1',
             'text': 'Test Text 1', 'slug': 'test-note-1'},
            {'login': False, 'title': 'Test Note 2',
             'text': 'Test Text 2', 'slug': 'test-note-2'},
            {'login': True, 'title': 'Test Note 3',
        ...
        for case in test_cases:
            with self.subTest(case=case):
                if case['login']:
                    self._login()
                response = self.client.post(self._get_url('add'),
                                            {'title': case['title'],
                                             'text': case['text']})
    ...
    def test_clean_slug_not_unique(self):
        self.note = Note.objects.create(title='Test Note',
                                        text='Test Text', author=self.user)
